...
fn monomorph_or_not() {
    ...
    fn strlen_generic<T>(v: T) -> usize where T: AsRef<str> {
        v.as_ref().len()
    }
...
mod tests {
    ...
    fn constructing_trait_object() {
        ...
        use std::mem::{size_of, size_of_val, align_of, transmute};
        ...
        let fat_pointer = unsafe { transmute::<(*const u8, *const usize), &dyn FastFood>(trait_obj) };
